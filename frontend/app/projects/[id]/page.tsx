...
export default function ProjectPage() {
  ...
  useEffect(() => {
    const fetchTodoItems = async () => {
      ...
      const { data, error } = await supabase.from("todo_items").select("*, user:users(email)").eq("project_id", id)
      if (error) console.error("Error fetching todo items:", error)
      else setTodoItems(data.map((item) => ({ ...item, user_email: item.user.email })) || [])
    }
    fetchTodoItems()
  }, [id])
  ...
  const addTodoItem = async () => {
    if (newItemTitle.trim() && user) {
      ...
      if (error) {
        ...
        toast({ title: "Error", description: "Failed to add todo item", variant: "destructive" })
      } else {
        setTodoItems([...todoItems, { ...data[0], user_email: user.email }])
        setNewItemTitle("")
        setNewItemEmoji("")
        setNewItemDeadline("")
        toast({ title: "Success", description: "Todo item added successfully" })
      }
    }
  }

  const updateTodoItem = async (item: TodoItem) => {
    const { error } = await supabase.from("todo_items").update(item).eq("id", item.id)
    if (error) {
      console.error("Error updating todo item:", error)
      toast({ title: "Error", description: "Failed to update todo item", variant: "destructive" })
    } else {
      setTodoItems(todoItems.map((i) => (i.id === item.id ? item : i)))
      setEditingItem(null)
      toast({ title: "Success", description: "Todo item updated successfully" })
    }
  }
  ...
  const deleteTodoItem = async (id: string) => {
    ...
    if (error) {
      ...
      toast({ title: "Error", description: "Failed to delete todo item", variant: "destructive" })
    } else {
      setTodoItems(todoItems.filter((i) => i.id !== id))
      toast({ title: "Success", description: "Todo item deleted successfully" })
    }
  }
  ...
  if (!user) {
    return (
      <Card className="p-6">
        ...
        <CardContent>
          <p>Manage your tasks efficiently with MyToDo. Create, update, and track your to-do items for each project.</p>
          ...
          <Card key={item.id} className="transition-all duration-200 hover:shadow-lg">
            <CardHeader>
              <CardTitle className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => updateTodoItem({ ...item, completed: checked as boolean })}
                />
            ...
            <CardContent>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-2 sm:space-y-0">
                ...
                <p className="text-sm text-muted-foreground">Created by: {item.user_email}</p>
                <div className="flex space-x-2">
                  ...
                  <Button variant="outline" size="icon" onClick={() => deleteTodoItem(item.id)}>
